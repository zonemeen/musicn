...
const search = async ({ text, options, serviceName }) => {
  ...
  if (serviceName === 'netease') {
    ...
    searchSongs = searchResult.result.songs
    if (searchSongs) {
      searchSongs = searchSongs.filter((item) => item.fee !== 1)
      for (const song of searchSongs) {
        const detailUrl = `https://music.163.com/api/song/enhance/player/url?id=ids=${song.id}&ids=[${song.id}]&br=3200000`
