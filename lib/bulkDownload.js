...
const download = (
  ...
  if (songName in oNames) {
    oNames[songName] += 1
    const [name, extension] = songName.split('.')
    const newName = `${name}(${oNames[songName]})`
  ...
  aBars.push(multiBar.create(songSize, 0, { file: songName }))
  ...
  const onError = (err) => {
    delUnfinishedFiles(targetDir, aUnfinishedPaths)
  ...
  fileReadStream.on('response', async () => {
    ...
    try {
      aUnfinishedPaths.push(songPath)
      await promisifyPipeline(fileReadStream, fs.createWriteStream(songPath))
      aUnfinishedPaths.splice(aUnfinishedPaths.indexOf(songPath), 1)
    } catch (err) {
      onError(err)
    }
  })
  ...
  fileReadStream.on('downloadProgress', ({ transferred }) => {
    aBars[index].update(transferred)
  })
